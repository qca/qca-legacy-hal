...
#undef IS
	...
	if (bChannelChange) {
		 ...
		 * it requires doubling writes to the RESET_TSF bit
		 * in the AR_BEACON register; it also has the quirk
	 ...
	 * rx_clear false triggering.  To correct this we add delay an
...
#define	N(a)			(sizeof (a) / sizeof (a[0]))
	 ...
	 * Limit the lowest value to 0.5 dBm. MIN_RATE_POWER = 1 in ar5212.h
	 */
                   ...
                   int16_t *pMinPower, int16_t *pMaxPower)
{
	...
	*pMinPower = MAX_RATE_POWER;
	...
	if (IS_CHAN_OFDM(chan)) {
		...
		if (ahp->ah_eeversion >= AR_EEPROM_VER4_0) {
			/* Setup XR target power from EEPROM. This
