...
/*
 * Note: these are not valid 2.4 channels, either we change these
 * or we change the beanie coding to accept these
 */
    ...
    struct hal_reg_parm *hal_conf_parm, HAL_STATUS *status)
{
	...
	default:
		if (AH_PRIVATE(ah)->ah_analog5GhzRev == 0) {
			 ...
			 * the RF chip rev so check the
			 * mac+phy revs and if srev is 9, set the radio rev
			 * to 0x46.
			 */
			if (AH_PRIVATE(ah)->ah_macVersion == AR_SREV_VERSION_VENICE &&
...
#define IS_LITE_CHIP(ah)  \
	...
	if (capField == 0 && IS_GRIFFIN_LITE(ah)) {
		/*
		 * This is for griffin-lite-lite cards with unprogrammed capabilities.
		 */
    ...
    /* 11n capabilities -- these are empty for AR5212 ! */
...
HAL_BOOL
ar5212DetectBbHang(struct ath_hal *ah, HAL_BOOL dfs_hang_check)
{
    return AH_FALSE;
}
